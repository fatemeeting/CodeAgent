...
6. Web 界面：python -m agent.web [--port 8080] [--workdir 目录]，浏览器打开 http://127.0.0.1:8080（整页工作区管理器：📂 原生选择 / 确认 / 最近列表 → 文件树 | Monaco 编辑器 | AI 聊天，三栏可拖拽）
...
- Web 界面：Cursor 风格 IDE——整页工作区管理器（选择 / 确认 / 最近列表）+ 文件树 | Monaco 编辑器 | AI 聊天（Markdown 渲染），三栏可拖拽，SSE 流式
- 过程可读：实时打印每步工具调用与结果
