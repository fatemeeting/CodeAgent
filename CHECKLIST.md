...
- [x] AQ1 前端「📚 技能装载」note 块；重放可见
- [x] AQ2 `GET/POST/PUT/DELETE /skills`（工作区级 CRUD、越界防护、只读标注）
- [x] AQ3 顶栏技能入口 + 技能管理浮层（浏览/选择/✎编辑/🗑删除/只读标签）；测试

## AT. 迭代 9 修复（/skill 命令与技能多选）

- [x] AT1 `/` 命令浮层仅保留 `/skill`（移除 `/skills` 命令项）；输入 `/skill` 后同浮层列出全部技能，点击累积多选（☑ 已选/再点取消）
