...
6. **Cursor 风格重写（v2）**：暖白视觉系统（`#f7f7f4` / `#26251e` / `#f54e00`、8px 间距）；**工作区管理器**（选择 / 确认 / 管理三要素 + 最近列表 + 目录校验，未选工作区不显示其它内容）；**Editor Window 布局**（左文件树 | 中 Monaco 代码编辑（CDN，文件名 tab 按需显示）| 右 AI 聊天 | 底部终端）；代码编辑器走 CDN Monaco（B 策略，前端运行时资源，不进入 Python 依赖）。
```
