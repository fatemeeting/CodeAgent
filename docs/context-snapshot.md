...
- 迭代 6 v2 布局精简通过：删除 Agent Window/模式切换与文件下拉框（仅 Editor Window：文件树 | Monaco | 聊天），文件名 tab 按需显示
- 迭代 6 v2 交互修复通过：三栏拖拽（localStorage 持久化）+ 文件树点击显示（Monaco 竞态修复 + CDN 兜底）；`pytest` 65 passed
- 迭代 6 v2 稳定性修复通过：逐栏 try/catch 隔离 + Monaco async 加载/双 CDN/超时回退（中/右栏显示恢复）；`pytest` 65 passed
- 迭代 6 v2 精简与美化通过：删除终端面板；聊天框美化（胶囊圆角发送/角色标签/消息置顶/空状态提示）；`pytest` 65 passed
...
- 迭代 7（backlog）：命令沙箱（真正隔离，替代模式匹配防护）/ 多 provider 切换
